...
# Поддерживаем старое имя функции, которое использовалось в предыдущих версиях
# скрипта. В некоторых установках оно всё ещё может вызываться напрямую,
# поэтому создаём явный алиас. Это исключает ошибку NameError, даже если
# скрипт загружен инкрементально и определение функции не считалось.
_negate_vector = _reverse_vector
